...
void makeFullPartitionRoom( BspNode& p, char( &map )[ WIDTH ][ HEIGHT ] )    //Fill partition with '*'
{
   int xMax = p.x + p.width;
   int yMax = p.y + p.height;
...
class Hallways
{
   ...
   int calculateDistanceFromCenterOfNodes( BspNode* startNode, BspNode* destinationNode )
   {
      ...
      //Pythagorean Theorem,  distanceBetweenRoomCenters = sqrt( a^2 + b^2 )
